/*
Откройте файл src/main.js и опишите в нём функции, возвращающие разметку элементов страницы (далее «компоненты»).
Вам потребуются функции для получения разметки следующих компонентов:

Информация о маршруте (доп.задание); (done)

Стоимость поездки (доп.задание); (done)
...
Форма создания/редактирования (используется одна форма); (done)
...
render(pageMainEvents, getCardEditorTemplate(), `beforeend`);
...
for (let i = 0; i < EVENTS_COUNT; i++) {
  render(tripEventList, getTripEventItem(), `beforeend`);
}
